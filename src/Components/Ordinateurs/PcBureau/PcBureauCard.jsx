...
const PcBureauCard = ({ product, onViewDetails }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-[#1e1e1e] rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-gray-100 dark:border-gray-700 h-full flex flex-col">
      ...
      <div className="relative overflow-hidden bg-gray-50 dark:bg-gray-700 h-32 sm:h-40 md:h-48">
        <img 
          src={product.image_url || product.image || 'https://images.unsplash.com/photo-1587831990711-23ca6441447b?w=400&h=400&fit=crop'} 
          alt={product.name || 'PC Gaming'}
        ...
        <div className="flex flex-col mb-3 sm:mb-4 mt-auto">
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-lg sm:text-xl font-bold text-green-600 dark:text-green-400">
              {product.price || product.discountPrice || '0'} MAD
            </span>
            {product.original_price && product.original_price > (product.price || product.discountPrice || 0) && (
              <span className="text-xs sm:text-sm text-gray-500 line-through">
                {product.original_price || product.originalPrice} MAD
              </span>
            )}
          </div>
          {product.original_price && product.original_price > (product.price || product.discountPrice || 0) && (
            <span className="text-xs text-green-600 dark:text-green-400 font-medium">
              Économisez {(product.original_price || product.originalPrice) - (product.price || product.discountPrice || 0)} MAD
            </span>
          )}
        </div>

        <button 
          id={`add-btn-${product.id || Math.random()}`}
